...
> **데이터 저장 위치:** 만든 시나리오는 그 컴퓨터·그 브라우저 안(IndexedDB)에 저장됩니다.
> 다른 PC와 공유하거나 백업하려면 **반드시 JSON 내보내기**를 사용하세요 (5장 참고).
...
| **결과 대기시간** | `보고 대기(분)` 만큼 실제 시간이 지나야 결과가 도착 (1~15분) |
| **회차(F/U)** | 같은 검사를 반복 처방하면 다음 회차 결과가 나옴. 설정한 회차를 초과하면 **검체불량** 처리(상태가 갑자기 호전된 것처럼 보이지 않도록) |
...
- **내보내기**: 설정 화면에서 각 시나리오의 `📤 내보내기` 또는 `📤 전체 내보내기` → `.json` 파일 생성
- **가져오기**: `📥 가져오기` → 받은 `.json` 파일 선택 (여러 개 동시 가능)
- 이 JSON 파일에는 **업로드한 영상 이미지까지 모두 포함**되므로, 파일 하나만 주고받으면 그대로 재현됩니다.
- 다른 PC에서도 **같은 주소(URL)로 접속 → JSON 가져오기** 하면 동일하게 사용할 수 있습니다.

> ⚠️ 브라우저 데이터를 지우면 저장된 시나리오가 사라집니다. **중요한 시나리오는 JSON으로 백업**해 두세요.
